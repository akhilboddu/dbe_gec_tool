...
export default function QuestionList({
  ...
  const handleRanking = async (score) => {
    fetchUserData().then(async () => {
      ...
      const docRef = doc(db, "ranking", assignmentId);
      const docSnap = await getDoc(docRef);
  ...
  const assignmentResultMutation = useMutation(
  ...
  const onSubmit = () => {
    if (!teacherId) {
      ...
      saveAssignmentScore(tempScore, textAnswersArr);
    } else {
      ...
      updateAttemptedAssignment(attemptData);
    }
  };
  ...
  const saveAssignmentScore = async (finalScore, textAnswersArr) => {
    fetchUserData().then(async () => {
      const { uid } = userData;
